...
This tool will get a quote for a DoorDash delivery. It requires dropoff details from the user (addresses, phone number) to return the fee and estimated delivery time. It
can optionally take in a business name.
"""
...
class GetDoorDashQuoteInput(BaseModel):
    ...
    physical_address: str = Field(
        ...,
        description=
        "The phsyical address of the user, e.g. `124 E 14th St, New York NY 10003`",
    )
    phone_number: str = Field(
        ...,
        description=
        "The phone number of the user in E.164 format, e.g. `+18605029731`",
    )
    # Add optional fields with default=None
    dropoff_business_name: str | None = Field(
        None, description="Optional business name for the dropoff location")


def get_door_dash_quote(physical_address: str,
                        phone_number: str,
    ...
    Args:
        ...
        dropoff_business_name (str): The business name of the dropoff location
    Returns:
    ...
    storage["external_delivery_id"] = external_delivery_id
    storage["phone_number"] = phone_number
    ...
    # Add optional fields to request_body if they are provided
    if dropoff_business_name:
        request_body["dropoff_business_name"] = dropoff_business_name
