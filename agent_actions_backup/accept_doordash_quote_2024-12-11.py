...
class AcceptDoorDashQuoteInput(BaseModel):
    ...
    dropoff_phone_number: str = Field(None, description="Dropoff phone number in E.164 format, e.g. `+18605029731`. If not provided, will use stored value from previous quote.")

def accept_door_dash_quote(external_delivery_id: str = None, tip: int = 0, dropoff_phone_number: str = None) -> str:
    ...
    dropoff_phone_number = dropoff_phone_number or stored_details.get("phone_number")
    ...
    try:
        ...
        # Clear memory after successful acceptance
